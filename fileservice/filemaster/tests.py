...
class ArchiveFileTest(TestCase):
    ...
    def setUp(self):
        ...
        for g in groups_for_regular:
            gr = Group.objects.get(name=g)
            res = self.c.put('/filemaster/groupsname/%s/' % gr.name, data='{"users":[{"email":"regularuser@thebeatles.com"}],"buckets":[{"name":"cbmi-fileservice-test"}]}',content_type='application/json')
            self.assertEqual(res.status_code, 200)
